...
const renderProductCards = (selectedBrand = "all", resetVisible = true) => {
    ...
    if (resetVisible) {
        currentVisibleCount = isBrandPage || isProductPage ? products.length : INITIAL_PRODUCTS_TO_SHOW
    }
    ...
    const displayedProducts = isProductPage
        ? searchedProducts
        : searchedProducts.slice(0, Math.min(currentVisibleCount, maxVisibleSearchCount))
    ...
    const productsToRender = isBrandPage ? displayedProducts : displayedProducts.slice(0, 4)

    productCard.innerHTML = productsToRender.map((product) => {
    ...
    if (moreButton) {
         moreButton.style.display = isProductPage ? "none" : "inline-flex"
       // moreButton.style.display = currentVisibleCount < maxVisibleSearchCount ? "inline-flex" : "none"
    }
    if (lessButton) {
         lessButton.style.display = isProductPage ? "none" : "inline-flex"
        ...
        <div class="summary-box p-4 rounded-4">
            ...
            <form class="checkout-form mt-4">
                ...
                <div class="mb-3">
                    ...
                    <input type="tel" name="phone" data-phone-input inputmode="numeric" pattern="[0-9]+" class="form-control" placeholder="Mobile number" required />
                </div>
                <div class="mb-3">
                ...
                <div class="mb-3">
                    ...
                    <textarea name="address" class="form-control" rows="3" placeholder="Street address, city, postcode" required></textarea>
                </div>
...
document.addEventListener("click", (event) => {
    ...
    if (loadMoreButton) {
        ...
        const maxVisibleCount = Math.min(filteredProducts.length, INITIAL_PRODUCTS_TO_SHOW + (PRODUCTS_PER_LOAD * MAX_MORE_LOADS))
        if (currentVisibleCount < maxVisibleCount) {
            currentVisibleCount = Math.min(currentVisibleCount + PRODUCTS_PER_LOAD, maxVisibleCount)
    ...
    if (loadLessButton) {
        currentVisibleCount = Math.max(currentVisibleCount - PRODUCTS_PER_LOAD, INITIAL_PRODUCTS_TO_SHOW)
        renderProductCards(currentBrandFilter, false)
        return
    }
    ...
    if (lineupButton) {
        ...
        currentVisibleCount = currentLineupFilter === "all"
